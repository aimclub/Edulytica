...
@pytest.mark.asyncio
@patch("src.edulytica_api.api.actions.fast_parse_text")
@patch("src.edulytica_api.api.actions.TicketCrud.create")
@patch("src.edulytica_api.api.actions.TicketTypeCrud.get_filtered_by_params")
@patch("src.edulytica_api.api.actions.TicketStatusCrud.get_filtered_by_params")
@patch("src.edulytica_api.api.actions.DocumentCrud.create")
@patch("src.edulytica_api.api.actions.DocumentCrud.get_by_id")
@patch("src.edulytica_api.api.actions.CustomEventCrud.get_by_id")
@patch("src.edulytica_api.api.actions.EventCrud.get_by_id")
@patch("builtins.open", new_callable=mock_open)
@patch("src.edulytica_api.api.actions.os.makedirs")
...
@pytest.mark.asyncio
@patch("src.edulytica_api.api.actions.EventCrud.get_filtered_by_params")
@patch("src.edulytica_api.api.actions.CustomEventCrud.get_filtered_by_params")
def test_get_event_id_success(mock_custom, mock_event, client):
    ...
    response = client(app).get("/api/actions/v1/get_event_id", params={"event_name": "test"})
...
def test_get_event_id_not_found(mock_custom, mock_event, client):
    ...
    response = client(app).get("/api/actions/v1/get_event_id", params={"event_name": "nonexistent"})
    assert response.status_code == 400
    assert response.json()["detail"] == "Event doesn't exist"


@pytest.mark.asyncio
@patch("src.edulytica_api.api.actions.TicketCrud.get_ticket_by_id_or_shared")
def test_get_ticket_success(mock_get_ticket, client):
    mock_get_ticket.return_value = "mock_ticket"

    response = client(app).get("/api/actions/v1/get_ticket", params={"ticket_id": str(uuid.uuid4())})
    assert response.status_code == 200
...
def test_get_ticket_file_success(mock_doc_get, mock_ticket_get, client, tmp_path):
    ...
    response = client(app).get("/api/actions/v1/get_ticket_file", params={"ticket_id": str(uuid.uuid4())})
    assert response.status_code == 200
...
@pytest.mark.asyncio
@patch("src.edulytica_api.api.actions.TicketCrud.get_ticket_by_id_or_shared")
@patch("src.edulytica_api.api.actions.DocumentSummaryCrud.get_by_id")
def test_get_ticket_summary_success(mock_summary, mock_ticket, client, tmp_path):
    ...
    response = client(app).get(
        "/api/actions/v1/get_ticket_summary",
        params={
            "ticket_id": str(
                uuid.uuid4())})
    assert response.status_code == 200
...
@pytest.mark.asyncio
@patch("src.edulytica_api.api.actions.TicketCrud.get_ticket_by_id_or_shared")
@patch("src.edulytica_api.api.actions.DocumentReportCrud.get_by_id")
def test_get_ticket_result_success(mock_report, mock_ticket, client, tmp_path):
    ...
    response = client(app).get(
        "/api/actions/v1/get_ticket_result",
        params={
            "ticket_id": str(
                uuid.uuid4())})
    assert response.status_code == 200
...
@pytest.mark.asyncio
@patch("src.edulytica_api.api.actions.TicketCrud.get_filtered_by_params")
@patch("src.edulytica_api.api.actions.TicketCrud.update")
def test_ticket_share_success(mock_update, mock_get, client):
    mock_get.return_value = MagicMock(shared=False)

    response = client(app).post("/api/actions/v1/ticket_share", json={"ticket_id": str(uuid.uuid4())})
    assert response.status_code == 200
