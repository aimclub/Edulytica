...
async def get_purpose(
    ...
    ticket = await TicketsCrud.create(session=session, ticket_type="Достижимость", user_id=user.id, status_id=0)
...
async def get_summary(
    ...
    ticket = await TicketsCrud.create(session=session, ticket_type="Суммаризация", user_id=user.id, status_id=0)
...
async def get_results(
    auth_data: Annotated[dict, Depends(access_token_auth)], session: AsyncSession = Depends(get_session)
):
    user = auth_data["user"]
...
async def get_result(
    ...
    try:
        ...
        if ticket is not None:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Not Permission")
