...
async def register_user(user: auth_schemas.UserCreate, session: AsyncSession = Depends(get_session)):
...
async def refresh_token(
    ...
    tokens = await TokenCrud.get_filtered_by_params(
        session=session, user_id=user.id, checker=payload["checker"], refresh_token=token
    )
    if len(tokens) != 1:
    ...
    await TokenCrud.update(
        session=session, record_id=token.id, refresh_token=refresh_token, checker=checker, status=True
    )
