...
def test_registration_success(mock_generate_code, mock_send_email, integration_client):
    ...
    response = integration_client.post("/registration", json=test_user_data)
...
def test_check_code_success(integration_client):
    response = integration_client.post("/check_code", json={"code": static_code})
    assert response.status_code == 200
...
def test_get_access_success(integration_client):
    ...
    response = integration_client.get("/get_access", cookies={
...
def test_logout_success(integration_client):
    ...
    response = integration_client.get("/logout", cookies={"refresh_token": refresh_token})
    assert response.status_code == 200
